...
//   --bare                        skip hooks, skills, auto-memory, keychain reads (clean CI mode)
...
//
// Auth: reads CLAUDE_CODE_OAUTH_TOKEN or ANTHROPIC_API_KEY from env.

export const claude: AgentRunner = {
  name: 'claude',
  async review(input: AgentInput): Promise<Review> {
    if (!process.env.CLAUDE_CODE_OAUTH_TOKEN && !process.env.ANTHROPIC_API_KEY) {
      return errorReview('claude', 'no CLAUDE_CODE_OAUTH_TOKEN or ANTHROPIC_API_KEY in env');
    }
    try {
      const args = [
        '-p',
